...
class TestConsolidateTaking:
    ...
    def test_get_one_warernhouse_all_categories(self):
        ...
        spected_data = {
            ...
            "owners": ["CORPORACIÓN PLUSBRAND DEL ECUADOR CIA. LTDA."],
            "sample_products": [
        ...
        for item in resume_taking["report"]:
            spected_item = next(
                (x for x in spected_data["sample_products"]
                 if x["account_code"] == item["account_code"]), None)
        ...
        assert len(resume_taking["report"]) == spected_data["total_products"]
        # comprobamos las empresas
        assert resume_taking["owners"] == spected_data["owners"]
